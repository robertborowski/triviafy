...
# ------------------------ individual function start ------------------------
def create_celebrations_test_function(postgres_connection, postgres_cursor, db_question_dict, i_celebrate_dict, i_product):
  ...
  fk_group_id = i_celebrate_dict['fk_group_id']
  timezone = 'EST'
...
def run_job_function():
  ...
  for i_product in products_arr:
    # ------------------------ lookup/celebrate start ------------------------
    db_arr_of_dict_if_celebration_today = None
    if i_product == 'birthday':
      db_arr_of_dict_if_celebration_today = select_manual_function(postgres_connection, postgres_cursor, 'select_if_birthday_today', current_month, current_day)
    if i_product == 'job_start_date':
      if int(current_day) == int(1):
        db_arr_of_dict_if_celebration_today = select_manual_function(postgres_connection, postgres_cursor, 'select_if_job_start_date_today', current_month, current_year)
      else:
        localhost_print_function(f"No anniversary job_start_date because today is not the first of the month, continue")
        continue
    # ------------------------ if no celebrate today start ------------------------
    if db_arr_of_dict_if_celebration_today == None or db_arr_of_dict_if_celebration_today == [] or len(db_arr_of_dict_if_celebration_today) == 0:
      print('no celebration today')
    # ------------------------ if no celebrate today end ------------------------
    # ------------------------ if yes celebrate today start ------------------------
    else:
      for i_celebrate_dict in db_arr_of_dict_if_celebration_today:
        ...
        if db_arr_of_dict_total_teammates[0]['count'] == 1:
          localhost_print_function(f"group_id: {i_celebrate_dict['fk_group_id']} = 1 total user. No test created because no one else to celebrate, continue.")
          continue
        # ------------------------ get total teammate count end ------------------------
        # ------------------------ get question obj start ------------------------
        db_arr_of_dict_question = select_manual_function(postgres_connection, postgres_cursor, 'select_question_1', i_celebrate_dict['fk_question_id'])
        db_question_dict = db_arr_of_dict_question[0]
        # ------------------------ get question obj end ------------------------
        # ------------------------ create test start ------------------------
        create_test_id = None
        new_test_created = False
        db_arr_of_dict_test_check = select_manual_function(postgres_connection, postgres_cursor, 'select_test_1', i_product, i_celebrate_dict['fk_question_id'])
        if db_arr_of_dict_test_check == None or db_arr_of_dict_test_check == []:
          create_test_id = create_celebrations_test_function(postgres_connection, postgres_cursor, db_question_dict, i_celebrate_dict, i_product)
        ...
        if new_test_created == True:
          ...
          insert_question_used_for_test_function(postgres_connection, postgres_cursor, i_celebrate_dict['fk_group_id'], i_celebrate_dict['fk_question_id'], create_test_id, i_product)
          # ------------------------ add question used to db end ------------------------
          # ------------------------ update celebrations db start ------------------------
          update_input_arr = [create_test_id, i_celebrate_dict['fk_question_id']]
        ...
        for i_teammate_dict in db_arr_of_dict_teammates:
          # ------------------------ skip self email start ------------------------
          if i_teammate_dict['email'] == db_user_dict['email']:
            localhost_print_function('skip sending email to self, continue')
            continue
          # ------------------------ skip self email end ------------------------
          if i_product == 'birthday':
            # ------------------------ send email start ------------------------
            ...
            if db_email_already_sent == None or db_email_already_sent == []:
              ...
              insert_inputs_arr = [send_email_id, send_email_created_timestamp, 'job_celebrate_email_'+i_product, i_teammate_dict['email'], output_subject, output_body]
            ...
            total_years_working_int = int(current_year) - int(i_celebrate_dict['celebrate_year'])
            ...
            if total_years_working_int < 1:
              localhost_print_function(f"Incorrect input year, continue")
              continue
            # ------------------------ get total years end ------------------------
            # ------------------------ send email start ------------------------
            ...
            # ------------------------ send email end ------------------------
        # ------------------------ loop teammates end ------------------------
    # ------------------------ if yes celebrate today end ------------------------
    # ------------------------ lookup/celebrate end ------------------------
