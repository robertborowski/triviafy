...
def job_daily_quiz_close_grade_winner_send_email_function():
  ...
  if today_day_of_week == 'Saturday' or today_day_of_week == 'Sunday':
   ...
   localhost_print_function('=========================================== job_daily_quiz_close_grade_winner_send_email_function END ===========================================')
   return True
  ...
  if latest_quiz_info_all_companies_arr != None:
    ...
    for company_latest_quiz_arr in latest_quiz_info_all_companies_arr:
      ...
      check_if_quiz_is_closed_datetime = check_if_today_is_greater_than_equal_to_latest_quiz_end_date_function(quiz_end_day_of_week, quiz_end_time)
      # ------------------------ Check Date Time Comparison END ------------------------


      # ------------------------ Pull Company User Info START ------------------------
      if check_if_quiz_is_closed_datetime == True:
        # ------------------------ Check If Lates Quiz Is Graded START ------------------------
        latest_quiz_is_fully_graded = check_if_latest_quiz_is_graded_function(quiz_slack_team_id, quiz_slack_channel_id, uuid_quiz)
        if latest_quiz_is_fully_graded != True:
          localhost_print_function('company user quiz answers have not yet been fully graded')
          localhost_print_function('=========================================== job_daily_quiz_close_grade_winner_send_email_function END ===========================================')
          # continue
          pass
        ...
        if this_weeks_winner_object == False:
          localhost_print_function('there is no winner this week, no one answered any questions or no one answered any questions correctly.')
        ...
        if check_if_slack_message_already_sent_to_company_user == None:
          ...
          output_message_content_str_for_db = send_team_channel_message_quiz_close_function(winner_user_slack_authed_id, user_slack_authed_incoming_webhook_url, quiz_channel_name)
...
if __name__ == "__main__":
  job_daily_quiz_close_grade_winner_send_email_function()