...
from website.backend.candidates.datetime_manipulation import get_current_weekday_function, get_current_hour_function
# ------------------------ imports end ------------------------
...
def create_quiz_function(group_id, immediate=False):
  ...
  if correct_cadence == True:
    ...
    if immediate == True:
      current_weekday = get_current_weekday_function()
      db_group_settings_dict['start_day'] = current_weekday
      current_hour = get_current_hour_function()
      db_group_settings_dict['start_time'] = current_hour
